/**
 * ChatGPT Provider for Surf Council
 * Wraps 'surf chatgpt' CLI command with timeout and zombie recovery.
 */
...
async function query(options = {}) {
  ...
  try {
    const result = await spawnWithTimeout(surfCommand, buildArgs(query, withPage, timeout), timeout);
    return {
      success: true,
...
function buildArgs(query, withPage, timeout) {
  const args = ["chatgpt", JSON.stringify(query), "--timeout", String(Math.floor(timeout / 1000))];
  if (withPage) {
    args.push("--with-page");
  }
  return args;
