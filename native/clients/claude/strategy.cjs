...
/**
 * @fileoverview Claude Strategy — CoT-aware with thinking block detection.
 *
 * Key difference from ChatGPT:
 *   If thinkingBlock selector finds visible content → return incomplete.
 *   We do NOT trust render-stable alone when thinking is present.
 *
 * Signal building (same as chatgpt but with thinkingBlock guard):
 *   isInteractionReady  → stopButton selector finds nothing
 *   isSemanticComplete  → doneToken selector finds visible element AND no thinking
 *   isRenderStable      → content length > 0 AND not in thinking phase
...
async function checkCompletion(ctx, _signals) {
  ...
  // ── CoT: Check for thinking block ───────────────────────────────────────────
  const thinkingChain = ctx.config?.selectors?.thinkingBlock ?? [];
  const thinkingVisible = findInContent(thinkingChain, pageContent).found;

  if (thinkingVisible) {
    // Force continue — do not trust render-stable when thinking is active
    return {
      done: false,
      reason: "Thinking block still visible — forcing continue",
      confidence: 0,
      activeSignals: ["thinkingBlock"],
    };
  }
