...
class PiChromeMcpServer {
  ...
  registerTools() {
    for (const [name, def] of Object.entries(TOOL_SCHEMAS)) {
      ...
      this.server.tool(name, def.desc, schemaObj, async (args) => {
        try {
  ...
  registerResources() {
    this.server.resource("page", "page://current", async (uri) => {
      try {
    ...
    this.server.resource("tabs", "tabs://list", async (uri) => {
      try {
    ...
    this.server.resource("console", "console://messages", async (uri) => {
      try {
    ...
    this.server.resource("network", "network://requests", async (uri) => {
      try {
